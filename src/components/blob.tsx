...
const BlobMesh: React.FC<BlobProps> = ({ color, gradientColors, useGradient, size, animation, intensity }) => {
  ...
  useFrame(() => {
    ...
    let scale = size; // Tamaño base

    // Aplica la escala según la animación
    if (animation === 'breathing') {
      scale *= Math.sin(intensity * performance.now() * 0.001) * 0.5 + 1; // Escala la animación
    }

    blobRef.current.scale.set(scale, scale, scale); // Asegura que la escala sea uniforme
  });
